...
const KNIGHT_MOVES_RAW: [(i8, i8); 8] = [
    (1, 2),
    (-1, 2),
    (2, 1),
    (-2, 1),
...
fn get_moves(board: &State, coord: (u8, u8), test_for_checks: bool) -> Vec<(u8, u8)> {
    ...
    if piece.kind.is_sliding() {
        ...
        moves = offsets_raw
            ...
            .filter(|s| {
                board[*s].content.is_none()
                    || board[*s].content.unwrap().colour == piece.colour.flip()
            })
            .collect::<HashSet<_>>();
        ...
        if piece.kind == Pawn {
            ...
            if board[up_coord].content.is_some() {
...
fn main() -> Result<(), String> {
    ...
    'running: loop {
        ...
        if let Some(coord) = state.selected_square {
            ...
            let valid_moves = get_moves(&state, coord, true);
            for y in 0..8_u32 {
                for x in 0..8_u32 {
